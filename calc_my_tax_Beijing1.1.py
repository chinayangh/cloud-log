...
bonus  = int(sys.argv[1][index_bonus+1:])
...
def calc_month_tax(income, before_tax):
    if income > 0:
        ...
        month_tax = income * r[2] - r[3] - before_tax
    else:
        r = (0, 0, 0, 0)
        month_tax = 0
    return (r, month_tax)
...
print_header()
print '                                  2019年北京市个人所得税计算方法（税前月薪为 %d，年终奖为%d）'%(salary, bonus)
print_header()
print "            应纳税额  ->  五险一金   起征点   专项扣除  适用税率    纳税    ->  到手工资  公积金    ->  年终奖  合并扣税  单独计税"
month_tax=0
...
while m <= 12:
    ...
    if m == 12:
        ...
        alone_bonus_tax = bonus * alone_bonus_r[2]
    else:
        month_bonus = 0
        merge_bonus_tax = 0
        alone_bonus_tax = 0
    ...
    total_tax += month_tax
    print_one_line(["%.f月"%(m), "%.f"%(salary), "->", "%.f"%(month_insurance), "%.f"%(starttax), "%.f"%(deduct), "%.f%%"%(r[2]*100), "%.f"%(month_tax), "->", "%.f"%(salary - month_tax-month_insurance), "%.f"%(month_fund), "->", "%.f"%(month_bonus), "%.f"%(month_bonus-merge_bonus_tax), "%.f"%(month_bonus-alone_bonus_tax)])
    m += 1
